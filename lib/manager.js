 ...
 *
 */
ConnectionManager.prototype.removeListeners = function (id) {
  if (! id) {
...
function add(client) {
  ...
  client.references.addRecord(this.__getScope());
...
ConnectionManager.prototype.remove = function (id) {
  ...
  if (client) {
    ...
    client.references.removeRecord(this.__getScope());
  } else {
    return;
  }

  if (ConnectionManager.prototype.referenceCount(id) <= 0) {
    //
    ...
    setTimeout(function () {
      ...
      } else if (ConnectionManager.prototype.referenceCount(id) <= 0) {
        // disconnect client
        try {
          console.log('ending client ' + id);
          console.log('references: ', client.references.getIdentifiers());
          client.disconnect.apply(self.buildScope(), function () {
            this.clients.removeRecord(id);
          });
        } catch (e) {
          throw new Error(e);
        }
 ...
 *   boolean
 */
ConnectionManager.prototype.move = function (oldID, oldcreds, newID, newcreds) {
